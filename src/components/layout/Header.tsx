...
import { Heart, Menu, Search, ShoppingCart, User, X, Package } from "lucide-react";
import { useAuth } from "@/hooks/useAuth";
import { useCart } from "@/hooks/useCart";
import { DEPARTMENTS } from "@/lib/departments";
import { PinCheck } from "@/components/store/PinCheck";
  ...
  return (
    <header className="sticky top-0 z-40 border-b border-border bg-background/95 backdrop-blur">
      <div className="bg-gold text-white">
        <div className="mx-auto flex max-w-7xl flex-wrap items-center justify-center gap-x-3 gap-y-0.5 px-4 py-1.5 text-[11px] font-medium sm:text-xs">
          <span>Retail delivery across Delhi</span>
          ...
          <Link to="/bulk-orders" className="underline underline-offset-2 hover:opacity-90">
            Bulk orders shipped pan-India
          </Link>
        </div>
      </div>
      ...
      <div className="mx-auto grid max-w-7xl grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-3 px-4 py-3">
        <div className="flex min-w-0 items-center gap-2">
          ...
          <PinCheck className="hidden xl:block" />
        </div>
        ...
        <form onSubmit={submitSearch} className="relative hidden min-w-0 md:block">
          ...
          <input
            ...
            placeholder="Search pressure cookers, bottles, jars or a product code"
      ...
      {/* Department strip: the primary navigation. Brands are a filter, not a
          top-level destination — a shopper looks for "a water bottle", not for
          "Dubblin's vacuum range". */}
      <div className="hidden border-t border-border lg:block">
        <div className="mx-auto flex max-w-7xl items-center gap-5 px-4 py-2 text-sm font-medium">
          <Link to="/shop" className="whitespace-nowrap hover:text-gold">
            All products
          </Link>
          {DEPARTMENTS.map((d) => (
            <Link
          ...
          <Link
            to="/bulk-orders"
            className="ml-auto flex items-center gap-1 whitespace-nowrap font-semibold text-gold hover:underline"
          >
            <Package className="h-4 w-4" />
            Bulk orders
          </Link>
          {isAdmin && (
            <Link to="/admin" className="whitespace-nowrap font-semibold text-gold">
              Admin
            </Link>
          )}
        </div>
